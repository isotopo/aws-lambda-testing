...
const getMessage = (handler) => `Handler is not a function: ${typeof handler}`;
...
module.exports = class extends MemoryUsage {
    ...
    exec(_params, callback) {
        ...
        if (typeof this.handler !== 'function') return Promise
            .reject(new Error(getMessage(this.handler)));

        this.setCallback(callback);
        this.removeAllListeners(eventTimeOutBrokenName);
